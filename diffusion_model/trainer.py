...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, condition_tensors = None, noise = None):
        b, c, h, w = x_start.shape  # Changed from b, c, h, w, d to b, c, h, w
        noise = default(noise, lambda: torch.randn_like(x_start))
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                # Convert to 2D image and save as PNG
                sample_image = all_images[0].cpu().numpy()  # Take first sample
                
                # Handle different channel configurations
                if sample_image.shape[0] == 1:  # Single channel (grayscale)
                    sample_image = sample_image[0]  # Remove channel dimension
                    # Normalize to 0-255 range
                    sample_image = (sample_image + 1.0) * 127.5
                    sample_image = np.clip(sample_image, 0, 255).astype(np.uint8)
                    pil_image = Image.fromarray(sample_image, mode='L')
                elif sample_image.shape[0] == 3:  # RGB
                    sample_image = np.transpose(sample_image, (1, 2, 0))  # CHW to HWC
                    # Normalize to 0-255 range
                    ...
                    sample_image = (sample_image + 1.0) * 127.5
                    sample_image = np.clip(sample_image, 0, 255).astype(np.uint8)                    
                    pil_image = Image.fromarray(sample_image, mode='RGB')
                else:
                ...
                pil_image.save(str(self.results_folder / f'sample-{milestone}.png'))
